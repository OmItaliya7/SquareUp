...
const Header = () => {
  ...
  return (
    <>
      ...
      <header className="w-full lg:border-b border-border-subtle">
        ...
        <div className="flex items-center justify-between pt-10 pb-5 xl:py-5 layout-shell">
          <Link to="/" onClick={closeMenu} className="flex items-center gap-2 shrink-0"  >
            ...
            />
            <span className="font-semibold text-text-primary 2xl:text-xl">
              SquareUp
            </span>
          </Link>
          ...
          <nav aria-label="Main navigation" className="items-center hidden lg:flex lg:gap-1">
            {navLinks.map((item) => (
              <Link
                key={item.name}
                to={item.path}
                className={`px-5.5 py-3 rounded-lg text-sm transition-all 2xl:px-7 2xl:py-3.5 2xl:text-lg ${
                  isActive(item.path)
          ...
          {/* Desktop Contact */}
          <Link to="/contact" className="hidden rounded-md bg-accent-primary px-4.5 py-3 text-sm font-medium text-text-tertiary lg:flex 2xl:px-6 2xl:py-4 2xl:text-lg">
            Contact Us
          </Link>
      ...
      {/*  MOBILE MENU  */}
      <div id="mobile-menu" className={`absolute z-40 w-full border-b border-border-subtle bg-bg-primary transition-all duration-300 lg:hidden ${open ? "max-h-125 py-6" : "max-h-0 overflow-hidden" }`}>
            ...
            <Link
              ...
              className={`rounded-md px-4 py-3 text-sm ${
                isActive(item.path)
                  ...
                  : "font-medium text-text-muted hover:bg-bg-secondary hover:text-white"
              }`}
            >
              {item.name}
            </Link>
