...
const ServiceCategoryGrid = ({ categoryTitle, items }) => (
  <>
    {/* Category heading  */}
    <div className="p-10 2xl:p-12.5 border-b border-subtle">
      <h3 className="font-medium text-xl xl:text-[22px] 2xl:text-[28px] text-muted text-center md:text-left">
        {categoryTitle}
      </h3>
    </div>
    ...
    {/* Cards grid  */}
    <div className="grid grid-cols-2 xl:grid-cols-4 xl:divide-x divide-subtle">
      {items.map((item) => (
        <ServiceCard key={item.id} icon={item.icon} title={item.title} id={item.id} />
      ))}
    </div>
  </>
);
