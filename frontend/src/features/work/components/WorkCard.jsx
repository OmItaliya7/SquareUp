...
const WorkCard = ({ category, image, brand, url, description }) => {
  return (
    <div className="flex flex-col gap-5 pb-10 xl:pb-20 xl:gap-7.5 2xl:pb-25">
      ...
      {/*  TITLE */}
      <h4 className="
        px-6 xl:px-10 2xl:px-12.5
        py-5 xl:py-6 2xl:py-7.5
        border-b border-border-subtle
        text-text-muted text-lg xl:text-[20px] 2xl:text-[26px] font-medium
      ">
        {category}
      </h4>
      ...
      ">
        ...
        <div className="flex items-end justify-between lg:items-center">
          ...
          <div className="flex flex-col gap-2.5">
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="w-fit px-2.5 py-2 xl:px-3 2xl:px-3.5 2xl:py-2.5 rounded-md 2xl:rounded-lg bg-bg-secondary border border-border-subtle text-text-muted text-sm xl:text-base 2xl:text-xl hover:text-text-accent transition-colors"
            >
              {url}
            </a>
          </div>
        ...
        {/* DESCRIPTION */}
        <p className="text-text-muted text-[14px] xl:text-base 2xl:text-lg">
          {description}
        </p>
